...
@pytest.mark.parametrize(
    "release_commits, type_regex, sec", case["test_generate_section"]
)
...
@pytest.mark.parametrize(
    "release_commits, part_name, release_body", case["test_generate_release_body"]
)
def test_generate_release_body(release_commits, part_name, release_body):
    assert release_body == main.generate_release_body(
        release_commits, part_name, "general", False
    )

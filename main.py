...
# @LastEditTime : 2025-08-25 16:40:19
...
class GithubChangelog:
    ...
    def __init__(
        ...
        commit_message: str,
        committer: str,
        unreleased_commits: bool,
        regenerate_count: int,
        part_name,
        default_scope: str,
        suppress_unscoped: bool,
        replace_empty_release_info: str,
    ):
        """
        ...
        self.author = (
            ...
            if committer == ""
            else github.InputGitAuthor(committer.split(" ")[0], committer.split(" ")[1])
        )
    ...
    def get_data(self):
        ...
        if self.regenerate_count < 0:
            ...
            regenerate_releases = regenerate_releases[0 : (self.regenerate_count + 1)]
        for r in releases:
            if (
                r.tag_name not in self.release_in_changelog
                and r.tag_name not in regenerate_releases
            ):
        ...
        logger.info(f"Regenerate releases: {regenerate_releases}")
        # get tags
        ...
        for release in releases:
            commit_sha = (
                tags_sha[release.tag_name] if release.tag_name in tags_sha else ""
            )
            release_content = (
                self.release_in_changelog[release.tag_name]
                if release.tag_name in self.release_in_changelog
                and release.tag_name not in regenerate_releases
                else ""
            )
    ...
    def get_exist_changelog(self):
        ...
        try:
            ...
            if isinstance(contents, list):
                logger.warning("Get a list of contents, take the first one.")
                contents = contents[0]
    ...
    def analyze_changelog(self):
        # analyze changelog
        body_content = ""
        if self.changelog.startswith(BEGIN_CHANGELOG_TITLE) and self.changelog.endswith(
            END_CHANGELOG_SIGNATURE + "\n"
        ):
            body_content = self.changelog[
                len(BEGIN_CHANGELOG_TITLE) : -len(END_CHANGELOG_SIGNATURE + "\n")
            ]
        else:
            if self.changelog != "":
                logger.warning(
    ...
    def get_release_content(self, release_tag, commits):
        ...
        try:
                ...
                if message_head[-3:] == "..." and len(message) > 1:
                    if message[1][0:3] == "...":
                        message_head = re.sub(
                            r"  ",
                            r" ",
                            message_head[:-3] + " " + message[1].split("\n")[0][3:],
                        )
            ...
            if e.status == 403:
                ...
                logger.error(
                    f"Failed to get release content, message: {e.data}", e.data
                )
                return "", status_code
            else:
                status_code = e.status
            ...
            release_content = generate_release_changelog(
                self.releases[release_tag],
                selected_commits,
                release_tag,
                self.part_name,
                self.default_scope,
                self.suppress_unscoped,
        ...
        if changelog == self.changelog:
            ...
            if self.file_exists:
                ...
                try:
                    ...
                    if e.status == 404:
                        new_sha = self.repo.get_branch(self.pull_request).commit.sha
                        self.repo.create_git_ref(f"refs/heads/{self.branch}", new_sha)
                        # get file content
                        try:
                            contents = self.repo.get_contents(self.path, self.branch)
                        except github.GithubException as e:
            ...
            logger.info(f"BRANCH: {self.branch}, PULL_REQUEST: {self.pull_request}")
            if self.pull_request != "" and self.pull_request != self.branch:
                logger.info(
                    f"Create pull request from {self.branch} to {self.pull_request}"
                )
...
def generate_release_changelog(
    ...
    release_body = generate_release_body(
        release_commits, part_name, default_scope, suppress_unscoped
    )
...
def main():
    ...
    if args.mode == "local":
        ...
        logger.error("Illegal mode option, please type '-h' to read the help")
        return
